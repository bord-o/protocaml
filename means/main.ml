...
type req = { op : operation; i : int32; j : int32 }
type asset_price = { timestamp : int32; price : int32 }

let display_of_req r =
  let { op; i; j } = r in
  let ds =
    (if op = Query then "Query" else "Insert")
    ^ " " ^ Int32.to_string i ^ " " ^ Int32.to_string j
  in
  ds
    ...
    | Query ->
        (let time1 = n1 in
         ...
         let table_aux timestamp price acc =
           if timestamp <= time2 && timestamp >= time1 then
             Int32.to_int price :: acc
           else acc
         in

         let results = Hashtbl.fold table_aux db [] in
         let mean =
           try Int.div (List.fold_left ( + ) 0 results) (List.length results)
