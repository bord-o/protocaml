...
let _bigreq =
  "{\"method\":\"isPrime\",\"number\":929627819581997065843982472548109446544378918505760532598,\"bignumber\":true}"
...
let string_parse (json : string) =
  let s =
    ...
    with _ -> (* any parsing exception counts as malfomed *)
              raise InvalidJson
  in
  s

let pmethod json = json |> member "method" |> to_string_option
...
let validate_fields meth num =
  match (meth, num) with
  | Some m, Some _ when not @@ String.equal m "isPrime" ->
      raise @@ IncorrectMethod m
  | Some m, Some n -> (m, n)
  | None, Some _ -> raise @@ Malformed "method"
  | Some _, None -> raise @@ Malformed "number"
  | _, _ -> raise @@ Malformed "all fields"

let send_json (json : string) flow = Eio.Flow.copy_string json flow
let send_malformed flow = send_json malformed_payload flow

let send_success is_prime flow =
  send_json
    (Printf.sprintf "{\"method\": \"isPrime\", \"prime\": %b}\n" is_prime)
    flow
...
let rec handle_client buf flow =
  ...
  try
    ...
    let is_it_prime =
      if Float.is_integer rnumber then
        let parsed_number = Float.to_int rnumber in
        Isprime.isPrime parsed_number
      else false
    in
...
let main ~net ~addr =
  let main_error e =
    match e with
    | End_of_file -> ()
    | _ -> Eio.traceln "Error in run_server %a" Fmt.exn e
  in
...
let () = Eio_main.run @@ fun env -> main ~net:(Eio.Stdenv.net env) ~addr
